...
```
이상의 코드를 해석하자면 일단 do 블록 내의 코드가 먼저 실행된 후, while문의 조건을 비교해 조건식이 참인 경우 다시 do블럭의 코드가 실행됩니다. 
...
for나 while 대신에 사용할만한 특별한 경우를 찾기가 어렵다는 점 떄문입니다. 

소스 코드에서 확인하게 됬을 때, 해석을 할 수만 있으면 됩니다. 
...
#### 특정 index에 값 넣기
arr02[1]='딸기'; 와 같은 방식으로 사용 가능 함 -> 자바랑 똑같습니다. 
...
이상의 코드는 lottoArray 배열에서 index 위치 기준 1개 만큼의 요소를 제거한다는 의미로, 1만 제거한다느 것은 '인덱스 위치에 있는 요소만 제거한다고 해석할 수있습니다. 
...
for(let i=0; i<6; i++){
  ...
  //이상의 코드에서 45 안 쓴 이유는? -> 리터럴을 사용 지양
...
alert()/prompt() 와 같은 방식(알림창)이 아니라면, 브라우저에 있는 내용을보여주기 위해서는 기본적으로 <body>태그 내에 이썽야만 합니다. 

##### HTML 안의 특정 태그에 내용을 넣는 방법
1. document.write(); ->  지난번에 수업함.
2. DOM 엘리먼트를 직접 제어하는 방법.

: 2.의 방법은 작성해야할 코드가 늘어나기는 하는데, 브라우저가 혀용하는 대부분의 방식을 제어할 수 있습니다. 저희가 확인할 수 있는 대부분의 웹 사이트가 해당 방법을 사용합니다. 

하지만 브라우저가 직접 제공하는 DOM 제어 API는 매우 기초적인 수준이라서 DOM 제어를 도와주는 자바 스크립트 라이브러리를 이용합니다. 
...
script2.js에 변수 명들에 전부다 2를 갖다붙였습니다. 
...
```
이렇게 처리했습니다. 
콘솔에 찍히는 부분은 script.js, 브라우저에 나오는 부분은 script2.js에 해당
...
```
이상의 의사 코드를 해석한 규칙:
1. 두 개의 값(a,b)를 받아서 '특정 기준'에 따라 비교했을 때, 
2. a가 더 작으면 '0보다 작은 값',
3. a가 더 크면 '0보다 큰 값'을 return
4. 2,3에 해당되지 않는다면(a,b가 같은 값이라면 )'0을 return'

이상의 방식을 기준으로 임의의 비교함수를 만들어 sort() 함수에 전달ㄷ하면, 브라우저는 그 comparator를 사용하여, 배열을 정렬합니다. 
...
3. __Comparator를 실행한 후 결과값을 sort()에 전달하는 것이 아니라 함수 자체를 전달하여 sort()가 알아서 Comparator를 실행하도록 한 것을 의미함__
...
즉 배열을 졍렬한다는 것의 의미는 : 비교함수를 어떻게 구현하는 가와 관련이 있습니다.

숫자 배열 뿐만 아니라, 문자열의 배열, 날짜의 배열, 혹은 오브젝트의 배열이라고 하더라도 비교함수만 맞춰서 만든다면 원하는 대로 정렬이 가능합니다. 

물론 항상 만능은 아닙니다...

: a와 b에 관계없이 return 값이 다르게 나온다면 sort() 함수가 정렬을 제대로할 수 없습니다. 
...
한번 쓰고 마는 경우에 익명함수(anonymous function)을 사용합니다. 
