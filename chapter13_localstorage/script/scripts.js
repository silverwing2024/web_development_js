...
function renderTodos(todo){
  ...
  todos.forEach((todo, index)=>{
    ...
    //삭제 버튼은 renderTodos()를 호출 했을 때 <li>가 만들어지고, <Button>이 만들어지기 때문에
    //기능과 관련된 부분이 없습니다. -> 즉 renderTodos() 내부에 button의 삭제 기능을 구현해둬야 할겁니다. 

    //물론 우리가 배운대로 외부에구현해놓고 여기서 호출만 해도 되는데 메카니즘을 설명하기 위해 여기에 작성하겠습니다. 

    deleteBtn.onclick = () => removeTodo(index); //버튼을 클릭하면 removeIndex() 함수를 호출하겠다
    // argument 로 index를 받았습니다. -> 즉 특정 요소 하나만 ㅎ삭제하는 버튼이라고 볼 수 있겠습니다. 
...
function removeTodo(index){
  //배열명.splice(삭제할 요소의 인덱스 장비, 제거할 요소의 숫자);
  todos.splice(index, 1); //버튼이 눌러진 index 넘버의 element만 삭제 -> 배열에서만 삭제
  //'배열에서만의 삭제'가 의미하는 바? -> 로컬 스토리지에는 남아있다 -> 그렇다며 다시 읽어오게 된다면
